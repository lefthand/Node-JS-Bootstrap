...
validatePostData = function (req, callback) {
  ...
  if (errors.length > 0) {
    ...
    data.content = req.param('postText');
    ...
    if (!req.user._id && !req.param('_id')) {
      ...
      if (!req.param('email_address')) {
        ...
        userDb.findOne({email: req.param('email_address')}, function( error, user) {
...
exports.add_routes = function (app) {
      ...
      if (error) {
        ...
        postDb.save( data, function( error, post) {
          ...
          if (data.requires_verification) {
            var verifySalt = bcrypt.genSaltSync(10);  
            var verifyHash = bcrypt.hashSync(data.title+data.created_at, verifySalt);
            ...
            var verificationMessagePlain = "Hi! Go to this address to verify your post '" + post.title + "' " + verifyLink;
            verificationMessagePlain += " and when you're done with it, you can delete the post from this here: " + deleteLink ;
            // Add an edit link some day.

            server.send({
              text:    verificationMessagePlain, 
              from: 'Management <' + siteInfo.site_email  + '>',
              to: req.param('email_address'),
              subject: 'Verify Post',
              attachment: 
  ...
  app.get('/post/:id/verify', loadGlobals, loadPost, loadCategories, function(req, res){
    ...
    if (req.post && postId && verify) {
      if (req.post && req.post.title && bcrypt.compareSync(req.post.title + req.post.created_at, verify)) {
        ...
        log.info('Could not verify post ' + JSON.stringify(req.post) + '!');
        res.redirect('/post/verification_failed/' + req.params.id);
      }
    }
    else {
      log.info('Something went wrong while tring to verify post!');
