...
exports.add_routes = function (app) {
  ...
  app.get('/post/create', loadStuff, function(req, res){
    localScript = 'post_create.js';
    categoryProvider.findAll(function(error, categories) {
      res.render('posts/create', { title: 'New Post', post: {_id:'',title:'',category:'',content:''}, categories: categories, globals:req.globals });
    }, {name:1});
  });
  ...
  app.get('/post/:id/edit', loadStuff, function(req, res, next){
    localScripts = 'post_edit.js';
    postProvider.findBy_Id(req.params.id, function(error, post) {
      if (req.is_admin || req.user._id === post.user_id) {
        categoryProvider.findAll(function(error, categories) {
          res.render('posts/edit', { title: 'Post ' + post.title, post: post, categories: categories, globals:req.globals });
        }, {name:1});
      }
      else {
