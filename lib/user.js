...
resetPassword = function (email, callback) {
  ...
  , function( error, docs) {
      if (error) {
        ...
        callback(null, 'Password for ' + email + 'changed to ' + newPassword);
      }
  });
}
...
exports.add_routes = function (app) {
  ...
  app.post('/forgot-password', loadGlobals, function(req, res, next){
    email = req.param('email');
    if (email) {
      resetPassword(email, function (error, result) {
        if (error) {
          log.error(error);
        }
        else {
      ...
      res.redirect('/forgot-password/?passwordSend=true');
    }
    else {
      res.redirect('/forgot-password/?passwordSend=false');
    }
  });
